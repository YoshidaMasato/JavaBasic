...
public class PTra13_07 {
	...
	public static void main(String[] args) {
		 ...
		 * 上記を繰り返し行います
		 */
		boolean loseFlgSuperHero = false;
		boolean loseFlgSlime = false;

		while(loseFlgSuperHero == false && loseFlgSlime == false) {
			loseFlgSlime = slime.damage(superHero.attack());
			loseFlgSuperHero = superHero.damage(slime.attack());
		}

		// ★ 勝利した方の出力を行ってください。「○○は■■との戦闘に勝利した」
		if(loseFlgSuperHero != true) {
		...
		System.out.println(loseFlgSuperHero);
		System.out.println(loseFlgSlime);
	}
}
